...
$(document).ready(function() { 
	...
	function update_batch (link, new_status) {
		...
		var hidden_input = $("#release_"+batch_id);
			...
			update_css(link, new_status);
			console.log( "Set PCR_results_released = " + new_status + " [ Batch: " + batch_id + "]");
		return false;
	}

	function update_css (target_element, release_this_batch) {

		var link = $(target_element);

		if(release_this_batch == "YES"){
			link.text("To be Released");
			link.removeClass("btn-danger click_to_release");
			link.addClass("btn-default click_to_retain");
			return;
		}

		if(release_this_batch == "NO"){
			link.text("To be Retained");
			link.removeClass("btn-default click_to_retain");
			link.addClass("btn-danger click_to_release");
			return;
		}
	}
