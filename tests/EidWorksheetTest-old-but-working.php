...
{ 
    ...
    public function test_interpreteResults_Scenario1Z_isNegative()
    {
        ...
        // $sample = $wsm->updateSample( DBS_SAMPLE_ID,  []);
        // $this_test = $wsm->interpreteResults(DBS_SAMPLE_ID, NEGATIVE, '1Z');

        // $this->assertTrue($this_test['accepted_result'] == $wsm->quote(NEGATIVE));
    }
    ...
    public function test_interpreteResults_Scenario4XF_isNegative()
    {
        ...
        // $this->createSamplesForEidWorksheet();
        // $wsm = new WorksheetManager( new Worksheet );
        // $sample = $wsm->updateSample( DBS_SAMPLE_ID,  ['test_1_result' => FAIL, 'nTestsDone' => 1]);
        // $this_test = $wsm->interpreteResults(DBS_SAMPLE_ID, NEGATIVE, '4XF');
    ...
    public function interpreteResults($results, $expected_scenario)
    {
        ...
        $prev_results = $results;// Note: its after array_pop()
    ...
    //     $this_result = array_pop( $results );
    //     $prev_results = $this->fmtPreviousResults( $results );
    //     $nPrevTests = count($prev_results);

    //     $new_data = $prev_results + ['nTestsDone' => $nPrevTests];
    //     $sample = $wsm->updateSample( DBS_SAMPLE_ID, $new_data );
    //     $this_test = $wsm->interpreteResults(DBS_SAMPLE_ID, $this_result, $expected_scenario);
    //     $accepted_result = $this_test['accepted_result'];
