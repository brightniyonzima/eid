...
$code='code128';
	...
	$text=5656;
		...
		$img_path = $_REQUEST['text']. '.jpg';
		$drawing = new BCGDrawing($img_path, $color_white);
		$drawing->setBarcode($code_generated);
		$drawing->setRotationAngle($_REQUEST['rot']);
		$drawing->setDPI($_REQUEST['dpi'] == 'null' ? null : (int) $_REQUEST['dpi']);
		$drawing->draw();

		// 	if(intval($_REQUEST['o']) === 1) {	header('Content-Type: image/png');	}
		// elseif(intval($_REQUEST['o']) === 2) {	header('Content-Type: image/jpeg');	}
		// elseif(intval($_REQUEST['o']) === 3) {	header('Content-Type: image/gif');	}
