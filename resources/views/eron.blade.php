...
<?php
  ...
  $sql =   "SELECT  DISTINCT
            ...
            ORDER BY facility_name, batch_id";
...
<?php 
        ...
        $batches[ $current_batch_id ][] = $r->axnNo;
  ...
  @if( $SCD->test_result == 'SAMPLE_WAS_REJECTED')
    ...
    <tr>
      ...
      <td class="tg-031e" colspan="3"   rowspan="2">
        ...
        <tr>
          <td class="tg-031e">Reviewed By:</td>
          <td class="tg-031e">{{ strtoupper( $SCD->labtech_family_name ) }}, {{ $SCD->labtech_other_name }}</td>
        </tr>
...
<?php 
  ...
  if( $print_me ){// update DB: mark batches as printed

    $sql = "START TRANSACTION;";
    foreach ($batches as $this_batch_id => $samples_in_batch) {
      $scd_results  = new stdClass;
      $scd_results->n = count($samples_in_batch);
      $scd_results->sample_IDs = $samples_in_batch;
      $scd_json = json_encode($scd_results);

      $scd_sql = " printed_SCD_results = '$scd_json' ";
      $sql .= "UPDATE batches SET $scd_sql WHERE id = '$this_batch_id'; ";   
    }
